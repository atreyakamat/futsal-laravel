...
export async function GET(request: Request) {
  try {
    ...
    // `?::date` (not `DATE ?`) — the DATE keyword only accepts a literal
    // directly after it, not a bind parameter; using it with a placeholder is
    // invalid Postgres syntax (confirmed in production: "syntax error at or
    // near $2").
    ...
    reportServerError(err, { route: 'gst-documents/export' });
    return NextResponse.json({ success: false, message: 'Failed to generate export' }, { status: 500 });
  }
}
