  ...
  try {
    ...
    const approval = await createApprovalRequest({
      ...
      payload: {
        ...
        slots,
        reason: payload.reason ?? 'Slot block request from arena admin',
      },
      notes: payload.reason ?? 'Slot block request',
    });

    await createAdminAuditLog({
      action: 'BLOCK_SLOT_REQUESTED',
      requestedBy: context.id,
      arenaId,
      newValue: { bookingDate: payload.booking_date, slots, reason: payload.reason },
    });

    return NextResponse.json({ success: true, message: 'Block slot request submitted for approval', requestId: approval?.id });
  } catch (error) {
    console.error('Block slot request error:', error);
