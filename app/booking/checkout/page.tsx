...
export default async function CheckoutPage({ searchParams }: Props) {
  ...
  if (!arenaId || !date || (slots || []).length === 0) {
    const fallbackArena = arenaId ? await getArenaById(arenaId) : null;
    return (
      <div className="max-w-7xl mx-auto px-6 py-20 text-center">
        <h1 className="text-3xl sm:text-4xl font-black uppercase">Invalid checkout session.</h1>
        <Link href={fallbackArena ? `/arena/${fallbackArena.slug}` : '/'} className="text-primary mt-4 inline-block font-bold">
          {fallbackArena ? `Back to ${fallbackArena.name}` : 'Back to Home'}
        </Link>
      </div>
    );
  }
  ...
  if (entryMode === 'blocked') {
    return (
      <div className="max-w-7xl mx-auto px-6 py-20 text-center">
        ...
        <Link href={`/arena/${arena.slug}`} className="text-primary mt-4 inline-block font-bold">Back to {arena.name}</Link>
      </div>
    );
  }
