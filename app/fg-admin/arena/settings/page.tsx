...
export default function ArenaAdminSettingsPage() {
    ...
    async function fetchData() {
      try {
        ...
        const [profileRes, arenaRes] = await Promise.all([
          fetch('/api/fg-admin/arena/settings'),
          fetch('/api/fg-admin/arena/arena')
        ]);
  ...
  async function handlePasswordChange(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/fg-admin/arena/settings', {
        method: 'PUT',
  ...
  async function handleImageRequest(e: React.FormEvent) {
    ...
    if (!coverImage && !logoUrl) {
      setImgError('Please provide at least one image URL');
      return;
    }
    ...
    try {
      const res = await fetch('/api/fg-admin/arena/requests', {
        ...
        body: JSON.stringify({ 
          ...
          reason: 'Manager submitted new images for review.'
        }),
      });
      ...
      if (res.ok && data.success) {
        setImgSuccess('Image update request submitted to Super Admin!');
        setCoverImage('');
        setLogoUrl('');
      } else {
        setImgError(data.message || 'Failed to submit request');
      }
    } catch (err) {
      setImgError('Error processing request. Please try again.');
    } finally {
      setImgLoading(false);
    }
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-20 space-y-12">
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        ...
        {/* Image Request form */}
        <form onSubmit={handleImageRequest} className="glass-card space-y-6 self-start">
          <h2 className="text-2xl font-black uppercase italic text-primary">Request Image Update</h2>
          <p className="text-[10px] font-bold text-white/30 uppercase tracking-widest">
            Submit new cover or logo images for Super Admin approval.
          </p>
          ...
          <button className="btn-primary w-full" type="submit" disabled={imgLoading}>
            {imgLoading ? 'SUBMITTING...' : 'REQUEST UPDATE'}
          </button>
        </form>
      </div>
    </div>
